...
Starts from [Helsinki-NLP/opus-mt-fr-ha](https://huggingface.co/Helsinki-NLP/opus-mt-fr-ha). Turso plus ARPRIM and Open-Data are labeled, then training can drop scripture/UI and upsample street phrases. The saved folder is `models/fr-pul/` (`config.json` + `model.safetensors`) for sibling package `marian_flutter` (Candle).

```bash
python3 -m venv --system-site-packages .venv
.venv/bin/pip install -r requirements.txt
.venv/bin/python export_turso.py
.venv/bin/python inject_greetings.py   # Bonjour→Mbaɗɗaa, Salutations→Salminaango
.venv/bin/python train.py
.venv/bin/python demo.py --label street
python ../../../marian_flutter/scripts/convert_tokenizer.py models/fr-pul
```
